 ...
 * <br><br>
 * This is a port and Java adaptation of the Rust's <a href="https://doc.rust-lang.org/std/result/index.html">Rust's Result Type</a>.
 *
...
public class Result<T, E> {
    ...
    /**
     * Generates a Result&lt;T,Exception&gt; which will be Ok(Value) if valueGetter gives a value, or Err(Exception) if an exception is thrown while trying to get said value.
     * <br><br>
     * Example:
     * <pre>{@code
     * public class Main {
     ...
     *         Result<List<String>, Exception> result = Result.from(
     *             () -> Files.readAllLines(new File("NonExistentFile").toPath())
     *         );
     *         Result<String, Exception> asString = result.map(Object::toString);
    ...
    /**
     * Generates a Result as Ok(value).
     *
    ...
    /**
     * Generates a Result as Err(error).
     *
     ...
     * @return Result&lt;T, E&gt; transformed to Result&lt;T, O&gt;, where the error E is transformed into O using errorMapper.
     */
    public <O> Result<T, O> mapError(Function<E, O> errorMapper) {
        return !isOk ? Result.err(errorMapper.apply(error)) : Result.ok(value);
    }
    ...
    public T expect(String errorMessage) throws RuntimeException {
        if (isOk) {
            ...
            throw new RuntimeException(errorMessage + System.lineSeparator() + "called `Result::unwrap()` on an `Err` value");
        }
    }
    ...
    public E expectErr(String errorMessage) throws RuntimeException {
        if (!isOk) {
            ...
            throw new RuntimeException(errorMessage + System.lineSeparator() + "called `Result::unwrap()` on an `Ok` value");
        }
    }
     ...
     */
    public <O> Result<T, O> orElse(Function<E, Result<T, O>> res) {
        return isOk ? Result.ok(value) : res.apply(error);
    }
